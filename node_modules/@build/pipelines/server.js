...
const buildPipeline = transform => {
	...
	if (transform.typingsDest) {
		let typingsDest = $.path.join(buildPath, transform.typingsDest);
		let typingsName = transform.typingsBaseName;
		...
		let dts = stream.dts
			.pipe($.insert.transform((contents, file) => {
				...
				contents = contents.replace(/from "(.+?)"/g, (whole, path) => {
					return `from "${ $.path.normalize($.path.join(typingsName, path)).replace(/\\/g, "/") }"`;
				});
				contents = contents.replace(/\n/g, "\n\t").trim();

				return `declare module "${ $.path.join(typingsName, modulePath).replace(/\\/g, "/") }" {\n\t${ contents }\n}`;
			}))
...
gulp.task("build:server", () => {
	...
	for (let transform of pipelines) {
		merged.add(buildPipeline(transform));

		if (core.watch) {
			gulp.watch(transform.source, e => {
				buildPipeline(transform);
			});
		}
	}
