...
const sortTypings = pipeline => {
	const getRelevant = path => {
		const sourcedir = $.path.parse(pipeline.source).dir;
...
const buildPipeline = pipeline => {
	...
	const buildPath = core.out;

	const dest = $.path.join(buildPath, pipeline.dest);
	const sourcedir = $.path.parse(pipeline.source).dir;

	const entries = [pipeline.source];
	...
	if (pipeline.typingsDest) {
		const outDir = $.path.join(buildPath, $.path.dirname(pipeline.typingsDest));
		const outFileName = $.path.basename(pipeline.typingsDest);

		const typingsDest = $.path.join(buildPath, pipeline.typingsDest);
