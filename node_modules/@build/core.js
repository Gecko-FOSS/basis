...
const core = {
	...
	getPipelines(type) {
		...
		return pipelines.filter(pipeline => {
			let name = (pipeline.id || pipeline.name || pipeline.type || "unnamed").toLowerCase();

			if (type != null && pipeline.type !== type) {
				return false;
			}

			if (core.args.black && core.args.black[name]) {
				return false;
			}

			if (core.args.white && !core.args.white[name]) {
				return false;
			}

			if (pipeline.disabled) {
				if (core.args.gray) {
					if (!core.args.gray[name]) {
						return false;
					}
				} else {
					return false;
