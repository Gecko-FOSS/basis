...
const buildTransform = transform => {
	...
	let dts = stream.dts
		.pipe($.insert.transform((contents, file) => {
			...
			const matches = file.path.match(new RegExp(`^.+?${ sourcedir.replace(/\*/g, "").replace(/\/\/+/g, "/") }[/\\\\]?(.+)`));
			...
			contents = contents.replace(/from "(.+?)"/g, (whole, path) => {
				return `from "${ $.path.normalize($.path.join("guh", path)) }"`;
			});
			contents = contents.replace(/\n/g, "\n\t").trim();

			return `declare module "${ $.path.join("guh", modulePath) }" {\n\t${ contents }\n}`;
		}))
